...
<body>
		...
		<center>
			...
			<div class="col-md-4"> <!-- Image Noise -->
				...
				if  ($_SESSION['image_status'] == "berhasil")  
				{
					...
					{
						noise_gaussian($_SESSION['image_name_ori'], $_SESSION['image_name_noise']);
						?>
	...
	//Noise Gaussian
		function noise_gaussian($image_name, $image_name_noise) 
		{
			...
			$w = imagesx($image_source);
			$h = imagesy($image_source);
        ...
        // getting gray value of all surrounding pixels
					$pixel_up         = get_luminance(imagecolorat($image_source,$x,$y-1));
					$pixel_down       = get_luminance(imagecolorat($image_source,$x,$y+1)); 
					$pixel_left       = get_luminance(imagecolorat($image_source,$x-1,$y));
					$pixel_right      = get_luminance(imagecolorat($image_source,$x+1,$y));
					$pixel_up_left    = get_luminance(imagecolorat($image_source,$x-1,$y-1));
					$pixel_up_right   = get_luminance(imagecolorat($image_source,$x+1,$y-1));
					$pixel_down_left  = get_luminance(imagecolorat($image_source,$x-1,$y+1));
					$pixel_down_right = get_luminance(imagecolorat($image_source,$x+1,$y+1));
        ...
        // calculating the distance
					$grayscale = sqrt($conv_x*$conv_x+$conv_y*$conv_y);
        ...
        // creation of the new gray
					$new_grayscale = imagecolorallocate($final,$grayscale,$grayscale,$grayscale);

        // adding the gray pixel to the new image        
					imagesetpixel($final,$x,$y,$new_grayscale);            
				}
			}

			if ($_SESSION['image_ext'] == "jpg" || $_SESSION['image_ext'] == "jpeg") 
			{
				imagejpeg($final, $_SESSION['image_name_noise']);
			}
			elseif ($_SESSION['image_ext'] == "png") 
			{
				imagepng($final, $_SESSION['image_name_noise']);
			}
